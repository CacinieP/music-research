...
- **演进**:
  ...
  - **V5** (2025): 音频保真度、人声真实感和歌曲结构的重大飞跃。改进的混音/母带级质感、更好的乐器分离、增强的流派适应性、更长的连贯歌曲生成
- **能力**:
  - 文本到歌曲：从文本描述生成完整歌曲
  - 歌词输入：自定义或 AI 生成的歌词
  - 通过提示进行流派/风格控制
...
- 商业 SVS 工具，源于 DiffSinger 技术
- 提供高质量多风格合成和表现力控制
- 支持多样化歌唱流派（流行、歌剧、民谣）和多种语言
- 代表学术 SVS 研究的商业化路径

#### DiffSinger-Lite (2025)
- DiffSinger 的加速版本，使用一致性蒸馏和渐进蒸馏
- 在保持质量的同时减少扩散采样步数
- 实现接近实时推理，适用于交互式 SVS 应用
...
- 使用对比学习对齐视觉和音频表示，从视频输入生成音乐
- 多模态 Transformer 处理视频帧并通过跨模态注意力生成对应的音乐序列
- 确保音乐输出与视觉内容（情绪、节奏、动态）协调一致

#### M2UGen (多模态音乐生成)
- 多模态音乐生成框架，处理视频、文本和图像输入
- 编码器-解码器架构：多模态编码器捕获输入特征（视频帧、文本），音乐解码器生成音频 token 或 MIDI
- 生成与输入模态语义对齐的音乐
